...
//const winston = require('winston/lib/winston/config'); This was auto added when winston was installed.
...
logger.add(new winston.transports.File({ 
  ...
  filename: './menuService/menuService.log'
  })
);
...
exports.start = function(PORT, STATIC_DIR, DATA_FILE) {
  ...
  // log requests
  // app.use(logger('combined')); Commented by Santosh to make way for winston
  ...
  app.get(API_URL, function(req, res, _next) {
    ...
    //log.console("This would generate reference error, since log is not definded");

    if (restaurant) {
      if (restaurant.name == 'Saucy Piggy') {
        logger.error("This would be the log for the error, the restaurant" + restaurant.name + "is not good");
  ...
  fs.readFile(DATA_FILE, function(_err, data) {
    ...
    app.listen(PORT, function() {
      ...
      logger.info("Starting the menu service on http://localhost:" + PORT + "/");
    });
  });
};
